...
import { bulkImportConsultants, type ConsultantBulkImportResult, type ConsultantBulkImportRow } from '../../payrollApi';

type CsvRow = ConsultantBulkImportRow;

const REQUIRED_COLUMNS = ['firstName', 'lastName', 'email', 'jobTitle', 'startDate', 'clientName'];
...
function parseCsv(text: string): { rows: CsvRow[]; skippedLines: number } {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (!obj.firstName || !obj.lastName || !obj.email || !obj.jobTitle || !obj.startDate || !obj.clientName) {
      skippedLines++;
      continue;
    }
    ...
    rows.push({
      ...
      roleName: obj.roleName || undefined,
      deploymentSite: obj.deploymentSite || undefined,
      employmentType: obj.employmentType || undefined,
      monthlyFee: obj.monthlyFee ? Number(obj.monthlyFee) : undefined,
      currency: obj.currency || undefined,
      noticePeriodDays: obj.noticePeriodDays ? Number(obj.noticePeriodDays) : undefined,
    });
  }
...
function downloadTemplate() {
  ...
  const sample =
    'Keza,Manzi,keza.manzi@example.com,Software Consultant,2026-01-15,Bank of Kigali,0788123456,Engineering,,650000,Backend Engineer,Kigali HQ,FULL_TIME,900000,RWF,30';
  const blob = new Blob([`${header}\n${sample}\n`], { type: 'text/csv' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = 'consultants-bulk-upload-template.csv';
  a.click();
  URL.revokeObjectURL(url);
}

export function PayrollConsultantsUploadPage() {
  const [file, setFile] = useState<File | null>(null);
  const [preview, setPreview] = useState<CsvRow[] | null>(null);
  const [skippedLines, setSkippedLines] = useState(0);
  const [busy, setBusy] = useState(false);
  const [err, setErr] = useState<string | null>(null);
  const [result, setResult] = useState<ConsultantBulkImportResult | null>(null);
  ...
  const onFileSelected = async (f: File | null) => {
    ...
    try {
      ...
      if (rows.length === 0) {
        setErr('No valid rows found. Make sure the CSV has firstName, lastName, email, jobTitle, startDate and clientName columns.');
        return;
      }
      setPreview(rows);
    } catch (e) {
  ...
  const onImport = async (ev: FormEvent) => {
    ...
    setErr(null);
    try {
      const res = await bulkImportConsultants(preview);
      setResult(res);
      setPreview(null);
    } catch (e) {
      setErr(e instanceof Error ? e.message : String(e));
  ...
  return (
    <div className="rec-page">
      <header className="rec-page__header">
        <div>
          <h1 className="rec-page__title">Consultants Upload</h1>
          <p className="rec-page__sub">
            Bulk-onboard outsourced consultants — each row creates the employee record and their
            client deployment (assignment) together.
          </p>
        </div>
      ...
      <div className="rec-card" style={{ maxWidth: 640 }}>
        <p className="muted small" style={{ marginBottom: 12 }}>
          ...
          <strong>jobTitle</strong>, <strong>startDate</strong> (YYYY-MM-DD), <strong>clientName</strong>.
          Optional: <em>phone</em>, <em>department</em>, <em>clientId</em>, <em>basicSalary</em>,{' '}
          <em>roleName</em> (defaults to jobTitle), <em>deploymentSite</em>,{' '}
          <em>employmentType</em> (FULL_TIME/PART_TIME/FIXED_TERM), <em>monthlyFee</em>,{' '}
          <em>currency</em>, <em>noticePeriodDays</em>. Rows with an email that already belongs
          to an employee will be skipped.
        </p>
          ...
          <form onSubmit={onImport}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button type="submit" className="btn btn--primary" disabled={busy || !preview || preview.length === 0}>
                {busy ? 'Importing…' : `Import${preview ? ` ${preview.length} consultant${preview.length === 1 ? '' : 's'}` : ''}`}
              </button>
              {file && (
